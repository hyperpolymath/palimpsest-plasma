...
[IMPORTANT]
====
**Status: historical design lineage.** This document is the original
OCaml-typed sketch of the policy AST. The normative specification — as
implemented in Rust in `plasma-engine/src/` — is
link:engine-v0-design.adoc[engine-v0-design.adoc], which documents the
deliberate divergences (exhibits generalised to overlays, per-rule
severity, license-specific conditions replaced by `spdx-license-is`).
====
